...
public:
	void found(const HASH_MAP< char , std::vector<size_t> > &sPrevPoss)
	{
		const size_t unitLength = getUnitLength();
		const std:: vector<char> &seq = refSeq();

		std:: cout << "begin" << std:: endl;
		for (HASH_MAP<char, std::vector<size_t> >::const_iterator pi = sPrevPoss.begin(); 
				pi != sPrevPoss.end(); ++pi)
		{
			if (pi->first == 0) {
			...
			const std:: vector<size_t> &poss = pi->second;
